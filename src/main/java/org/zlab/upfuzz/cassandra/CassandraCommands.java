...
/**
 * TODO: 1. nested commands & scope // we could do it in a simple way without scope first... 2.
 * mutate() & isValid() // we implemented generateRandomParameter() for each type 3. user defined
 * type // we need to implement a UnionType, each instance of a UnionType could be a user defined
 * type 4. mutate methods. - Shouldn't be operated by user. - When we call the command.mutate,
 * (Conduct param level mutation) - It should pick one parameter defined in this command, and call
 * its mutation method. - Then for the rest command, it should run check() method to do the minor
 * modification or not.
 */
public class CassandraCommands {
     ...
     * <p>E.g.,
     *
     * <p>CREATE TABLE emp( emp_id int PRIMARY KEY, emp_name text, emp_city text, emp_sal varint,
     * emp_phone varint );
     */
    public static final boolean DEBUG = false;

    public static final List<Map.Entry<Class<? extends Command>, Integer>> commandClassList =
            new ArrayList<>();
     ...
     */
    public static final List<Map.Entry<Class<? extends Command>, Integer>> createCommandClassList =
            new ArrayList<>();

    public static final List<Map.Entry<Class<? extends Command>, Integer>> readCommandClassList =
            new ArrayList<>();
    ...
    /**
     * CREATE KEYSPACE [IF NOT EXISTS] keyspace_name WITH REPLICATION = { 'class' :
     * 'SimpleStrategy', 'replication_factor' : N } | 'class' : 'NetworkTopologyStrategy',
     * 'dc1_name' : N [, ...] } [AND DURABLE_WRITES = true|false] ;
     */
    public static class CREAT_KEYSPACE extends Command {
        ...
        public CREAT_KEYSPACE(State state, Object init0, Object init1, Object init2) {
            ...
            ParameterType.ConcreteType keyspaceNameType =
            ...
            Parameter replicationFactor =
                    replicationFactorType.generateRandomParameter(state, this, init1);
            this.params.add(replicationFactor); // [1]

            ParameterType.ConcreteType IF_NOT_EXISTType =
                    new ParameterType.OptionalType(
                            new CONSTANTSTRINGType("IF NOT EXISTS"),
                            null // TODO: Make a pure CONSTANTType
                            );
        ...
        public CREAT_KEYSPACE(State state) {
            ...
            ParameterType.ConcreteType replicationFactorType = new INTType(1, 4);
            Parameter replicationFactor =
                    replicationFactorType.generateRandomParameter(state, this);
            this.params.add(replicationFactor); // [1]
    ...
    public static class CREATE_TABLE extends Command {
         ...
         * mutating a parameter could depend on and the state updated by all nested internal
         * commands and other parameters. Note: Thus, we need to be careful to not have cyclic
        ...
        public CREATE_TABLE(
                State state, Object init0, Object init1, Object init2, Object init3, Object init4) {
            super();
            ...
            ParameterType.ConcreteType tableNameType =
                    new ParameterType.NotInCollectionType(
            ...
            Parameter tableName =
                    tableNameType.generateRandomParameter(cassandraState, this, init1);
            ...
            Parameter primaryColumns =
                    primaryColumnsType.generateRandomParameter(cassandraState, this, init3);
            ...
            Parameter IF_NOT_EXIST =
                    IF_NOT_EXISTType.generateRandomParameter(cassandraState, this, init4);
        ...
        public CREATE_TABLE(State state) {
            ...
            /**
             * Bool variable check whether the previous columns has any member that's already
             * specified as Primary Key - True - Shouldn't generate the third param - False - Should
             * generate
             *
             * <p>Impl this check as a type - Take a previous parameter as input - genRanParam() -
            ...
            Parameter primaryColumns =
                    primaryColumnsType.generateRandomParameter(cassandraState, this);
            params.add(primaryColumns); // [3]
        ...
        public String constructCommandString() {
            ...
            ParameterType.ConcreteType primaryColumnsNameType =
                    new ParameterType.StreamMapType(
                            null,
                            (s, c) -> (Collection) c.params.get(3).getValue(),
                            p -> ((Pair<Parameter, Parameter>) ((Parameter) p).value).left);
            Parameter primaryColumnsName =
                    primaryColumnsNameType.generateRandomParameter(null, this);

            String ret =
                    "CREATE TABLE "
                            + IF_NOT_EXIST.toString()
                            + " "
    ...
    public static class CREAT_INDEX extends Command {
        ...
        public CREAT_INDEX(State state) {
            ...
            ParameterType.ConcreteType indexNameType =
                    new ParameterType.NotInCollectionType(
                            new ParameterType.NotEmpty(STRINGType.instance),
                            (s, c) ->
                                    ((CassandraState) s)
    ...
    /**
     * INSERT INTO [keyspace_name.] table_name (column_list) VALUES (column_values) [IF NOT EXISTS]
     * [USING TTL seconds | TIMESTAMP epoch_in_microseconds]
     *
     * <p>E.g., INSERT INTO cycling.cyclist_name (id, lastname, firstname) VALUES
     * (c4b65263-fe58-4846-83e8-f0e1c13d518f, 'RATTO', 'Rissella') IF NOT EXISTS;
     */
    public static class CREATE_TYPE extends Command {
        ...
        public CREATE_TYPE(State state) {
            ...
            ParameterType.ConcreteType typeNameType =
                    new ParameterType.NotInCollectionType(
                            new ParameterType.NotEmpty(STRINGType.instance),
                            (s, c) ->
                                    ((CassandraState) s)
                                            .keyspace2UDTs.get(c.params.get(0).toString()),
                            null);
    ...
    public static class INSERT extends Command {
        ...
        public INSERT(State state, Object init0, Object init1, Object init2, Object init3) {
            ...
            ParameterType.ConcreteType columnsType =
                    new ParameterType.SuperSetType(
                            new ParameterType.SubsetType(
                                    null,
                                    (s, c) ->
                                            ((CassandraState) s)
            ...
            ParameterType.ConcreteType insertValuesType =
                    new ParameterType.Type2ValueType(
                            null,
                            (s, c) -> (Collection) c.params.get(2).getValue(), // columns
                            p -> ((Pair) ((Parameter) p).value).right);
            Parameter insertValues =
                    insertValuesType.generateRandomParameter(cassandraState, this, init3);
        ...
        public String constructCommandString() {
            ...
            Parameter keyspaceName = params.get(0);
            Parameter tableName = params.get(1);
            ParameterType.ConcreteType columnNameType =
                    new ParameterType.StreamMapType(
                            null,
        ...
        public Set<Command> generateRelatedReadCommand(State state) {
            ...
            if (cassandraTable != null) {
                ...
                for (int i = 0; i < primaryCols.size(); i++) {
                    ...
                    primaryValues.add(
                            ((Parameter) insertValues.get(columnsNames.indexOf(primaryCols.get(i))))
                                    .getValue());
                }
    ...
    /** ALTER TABLE [keyspace_name.] table_name [DROP column_list]; */
    public static class ALTER_TABLE_DROP extends Command {
                        ...
                        CassandraTable cassandraTable =
                                ((CassandraState) s)
                                        .getTable(
                                                c.params.get(0).toString(),
                                                c.params.get(1).toString());
                        return cassandraTable.colName2Type.size()
                                != cassandraTable.primaryColName2Type.size();
                    };

            ParameterType.ConcreteType dropColumnType =
                    new ParameterType.NotInCollectionType(
                            new ParameterType.InCollectionType(
                                    null,
                                    (s, c) ->
                            ...
                            null);
            Parameter dropColumn =
                    dropColumnType.generateRandomParameter(cassandraState, this, init2);
        ...
        public ALTER_TABLE_DROP(State state) {
            ...
            /**
             * FIXME: About the Predicate. Two ways Keep the retry times, if it retrys for many
             * times, throw a warning about the constraints? Retry a few times, if not success, it
             * fails.
             */
            ParameterType.ConcreteType dropColumnType =
                    new ParameterType.NotInCollectionType(
        ...
        public void updateState(State state) {
            ((CassandraState) state)
                    ...
                    .colName2Type
                    .removeIf(value -> value.toString().equals(this.params.get(2).toString()));
        }
    }
    ...
    public static class ALTER_TABLE_ADD extends Command {
        ...
        public ALTER_TABLE_ADD(State state) {
            ...
            /**
             * Add a column - Must not be in the original column list - Pair type <String, TYPEType>
             */
            ParameterType.ConcreteType addColumnNameType =
                    new ParameterType.NotInCollectionType(
                            new ParameterType.NotEmpty(STRINGType.instance),
                            (s, c) ->
                                    ((CassandraState) s)
            ...
            Parameter addColumnName =
                    addColumnNameType.generateRandomParameter(cassandraState, this);
            this.params.add(addColumnName);

            ParameterType.ConcreteType addColumnTypeType = CassandraTypes.TYPEType.instance;
            Parameter addColumnType =
                    addColumnTypeType.generateRandomParameter(cassandraState, this);
    ...
    /**
     * ALTER KEYSPACE keyspace_name WITH REPLICATION = { 'class' : 'SimpleStrategy',
     * 'replication_factor' : N | 'class' : 'NetworkTopologyStrategy', 'dc1_name' : N [, ...] } [AND
    ...
    public static class DROP_INDEX extends Command {
        public DROP_INDEX(State state) {
            ...
            ParameterType.ConcreteType IF_EXISTType =
                    new ParameterType.OptionalType(
                            new CONSTANTSTRINGType("IF EXISTS"),
                            null // TODO: Make a pure CONSTANTType
                            );
        ...
        public String constructCommandString() {
            ...
            sb.append("DROP INDEX ")
                    .append(params.get(3))
                    .append(" " + this.params.get(0) + "." + this.params.get(2).toString() + ";");
            return sb.toString();
        }
    ...
    public static class DROP_KEYSPACE extends Command {
        ...
        @Override
        public void updateState(State state) {
            ((CassandraState) state).keyspace2tables.remove(params.get(0).toString());
            ((CassandraState) state).keyspace2UDTs.remove(params.get(0).toString());
        }
    }
        ...
        public DROP_TYPE(State state) {
            ...
            ParameterType.ConcreteType typeNameType =
                    new ParameterType.InCollectionType(
                            ...
                            (s, c) ->
                                    ((CassandraState) s)
                                            .keyspace2UDTs.get(this.params.get(0).toString()),
                            null);
    ...
    /**
     * DELETE firstname, lastname FROM cycling.cyclist_name USING TIMESTAMP 1318452291034 WHERE
     * lastname = 'VOS';
     */
    public static class DELETE extends Command {
        ...
        public DELETE(State state) {
            ...
            ParameterType.ConcreteType whereValuesType =
                    new ParameterType.Type2ValueType(
                            null,
                            (s, c) -> (Collection) c.params.get(2).getValue(),
                            p -> ((Pair) ((Parameter) p).value).right);
        ...
        public String constructCommandString() {
            ...
            List<Parameter> whereColumns =
                    (List<Parameter>)
                            whereColumnsType.generateRandomParameter(null, this).getValue();
    ...
    /**
     * SELECT * | select_expression | DISTINCT partition FROM [keyspace_name.] table_name [WHERE
     * partition_value [AND clustering_filters [AND static_filters]]] [ORDER BY PK_column_name
    ...
    public static class SELECT extends Command {
        ...
        public String constructCommandString() {
            ...
            if (((List) params.get(3).getValue()).size() > 0) {
                ...
                for (int i = 0; i < whereColumns.size(); i++) {
                    sb.append(
                            whereColumns.get(i).toString() + " = " + whereValues.get(i).toString());
                    if (i < whereColumns.size() - 1) {
                        sb.append(" AND ");
                    }
                }
    ...
    /** This helper function will randomly pick keyspace and return its tablename as parameter. */
    ...
    /** This helper function will randomly pick one table and return its table name as parameter. */
