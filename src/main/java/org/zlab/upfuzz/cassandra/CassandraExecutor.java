...
public class CassandraExecutor extends Executor {
    ...
    public boolean isCassandraReady(String oldSystemPath) {
        ...
        try {
            isReady =
                    Utilities.exec(
                            new String[] {"bin/cqlsh", "-e", "describe cluster"}, oldSystemPath);
    ...
    public void startup() {
        ...
        env.put(
                ...
                "-javaagent:"
                        ...
                        + ",includes="
                        + classToIns
                        + ",output=dfe,address=localhost,sessionid="
                        + systemID
                        + "-"
                        + executorID
                        + "_original");
    ...
    public void upgradeteardown() {
        ...
        try {
            ...
            p.waitFor();
            in.close();
            System.out.println("new cassandra " + executorID + " shutdown successfully");
        ...
        try {
            commandSequence =
                    CommandSequence.generateSequence(
                            ...
                            null);
            // TODO: If it's generating read with a initial state, no need to generate with
            // createTable...
    ...
    public List<String> executeCommands(CommandSequence commandSequence) {
        ...
        try {
            if (cqlsh == null) cqlsh = new CassandraCqlshDaemon(Config.getConf().oldSystemPath);
    ...
    public List<String> newVersionExecuteCommands(CommandSequence commandSequence) {
        ...
        try {
            ...
            if (cqlsh == null) cqlsh = new CassandraCqlshDaemon(Config.getConf().newSystemPath);
            for (String cmd : commandList) {
                // System.out
                //
                // .println("\n\n------------------------------------------------------------\nexecutor command:\n"
    ...
    public int moveSnapShot() {
        ...
        ProcessBuilder pb =
                new ProcessBuilder("cp", "-r", oldFolderPath.toString(), newFolderPath.toString());
        pb.directory(new File(Config.getConf().oldSystemPath));
    ...
    /**
     * 1. Move the data folder to the new version Cassandra 2. Start the new version cassandra with
     * the Upgrade symbol 3. Check whether there is any exception happen during the 4. Run some
     * commands, check consistency
     *
     * <p>upgrade process... Also need to control the java version.
     */
    @Override
    public boolean upgradeTest() {
        /**
         * Data consistency check If the return size is different, exception when executing the
         * commands, or the results are different. Record both results, report as a potential bug.
         *
         * <p>A crash seed should contain: 1. Two command sequences. 2. The results on old and new
         * version. 3. The reason why it's different - Upgrade process throw an exception - The
         * result of a specific command is different
         */
        ...
        try {
            this.cqlsh = new CassandraCqlshDaemon(Config.getConf().newSystemPath);
        } catch (IOException e) {
            e.printStackTrace();
        } catch (InterruptedException e) {
            e.printStackTrace();
        ...
        for (Integer testId : testId2commandSequence.keySet()) {
            testId2newVersionResult.put(
                    testId, newVersionExecuteCommands(testId2commandSequence.get(testId).right));
        }
        ...
        for (Integer testId : testId2commandSequence.keySet()) {
            ...
            // System.out.println("old version size = " + oldVersionResult.size() + " new version
            // size = " + newVersionResult.size());
            ...
            if (newVersionResult.size() != oldVersionResult.size()) {
                ...
                failureInfo =
                        "The result size is different, old version result size = "
                ...
                for (int i = 0; i < newVersionResult.size(); i++) {
                    ...
                    if (oldVersionResult.get(i).compareTo(newVersionResult.get(i)) != 0) {
                        ...
                        if (oldVersionResult.get(i).contains("SyntaxException")
                                && newVersionResult.get(i).contains("SyntaxException")) {
                            continue;
                        }

                        // InvalidRequest
                        if (oldVersionResult.get(i).contains("InvalidRequest")
                                && newVersionResult.get(i).contains("InvalidRequest")) {
                            continue;
                        }
                        ...
                        String errorMsg =
                                "Result not the same at read sequence id = "
                                        + i
                                        + "\n"
                                        + "Old Version Result: "
        ...
        // System.out.println("Upgrade System comparing results = " + (endTime - startTime)/1000.  +
        // "s");
        // Shutdown
        // startTime = System.currentTimeMillis();
        upgradeteardown();
