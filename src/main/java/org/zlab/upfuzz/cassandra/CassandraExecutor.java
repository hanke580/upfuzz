...
public class CassandraExecutor extends Executor {
    ...
    public void startup() {
        ...
        cassandraProcessBuilder
                .directory(new File(Config.getConf().cassandraPath));
        cassandraProcessBuilder.redirectErrorStream(true);
        cassandraProcessBuilder.redirectOutput(
                Paths.get(Config.getConf().cassandraPath, "logs.txt").toFile());
        try {
    ...
    public void upgradeteardown() {
        ...
        try {
            ...
            in.close();
            System.out.println(
                    "Upgrade Cassandra " + executorID + " shutdown ok");
    ...
    public boolean upgradeTest() {
        ...
        pb.directory(new File(Config.getConf().upgradeCassandraPath));
        pb.redirectOutput(
                Paths.get(Config.getConf().upgradeCassandraPath, "logs.txt")
                        .toFile());
        // long startTime = System.currentTimeMillis();
