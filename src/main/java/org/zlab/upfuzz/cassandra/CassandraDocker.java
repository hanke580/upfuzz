...
public class CassandraDocker extends DockerMeta implements IDocker {
    ...
    public CassandraDocker(CassandraDockerCluster dockerCluster, int index)
        ...
        name = "cassandra-" + originalVersion + "_" + upgradedVersion +
    ...
    public boolean build() throws IOException {
        ...
        javaToolOpts = "JAVA_TOOL_OPTIONS=\"-javaagent:"
                ...
                ",output=dfe,address=" + hostIP +
                ",port=" + agentPort + ",sessionid=" + system + "-" +
                executorID + "_" + type + "\"";

        env = new String[] {
                "CASSANDRA_HOME=\"" + cassandraHome + "\"",
                "CASSANDRA_CONF=\"" + cassandraConf + "\"",
                javaToolOpts,
    ...
    public Process exec(String[] cmd) throws IOException, InterruptedException {
        String[] dockerCMD = Utilities
                .concatArray(new String[] { "docker", "exec", containerName },
                        cmd);
