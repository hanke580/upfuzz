...
public class Main {
    ...
    public static void main(String[] args) throws ParseException {
        ...
        Option clazzOption = Option.builder("class").argName("type").hasArg()
                .desc("start a dfe server or client or fuzzer").required().build();
        // Option actionOption = Option.builder("action").argName("action").hasArg().desc("start a dfe server or client")
        //         .required().build();
        Option configFileOption = Option.builder("config").argName("config").hasArg()
                .desc("Configuration file location").build();
