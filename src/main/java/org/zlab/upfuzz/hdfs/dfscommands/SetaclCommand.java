...
public class SetaclCommand extends DfsCommands {
    ...
    public SetaclCommand() {
        ...
        // {-m | -x <acl_spec>}
        Parameter mxaclSpec =
                new ConcatenateType(MandX, aclSpec).generateRandomParameter(null, null);

        // <path>
        Parameter path1 = new CONSTANTSTRINGType("path1").generateRandomParameter(null, null);

        // [{-b|-k} {-m|-x <acl_spec>} <path>]
        Parameter bkmxaclPath =
                new ConcatenateType(BandK, mxaclSpec, path1).generateRandomParameter(null, null);
        ...
        Parameter setACLPATH =
                new ConcatenateType(hyphenSet, aclSpec1, path2).generateRandomParameter(null, null);

        // [{-b|-k} {-m|-x <acl_spec>} <path>]|[--set <acl_spec> <path>]
        Parameter fullACLBKMXPATH =
                new ConcatenateType(bkmxaclPath, setACLPATH).generateRandomParameter(null, null);
