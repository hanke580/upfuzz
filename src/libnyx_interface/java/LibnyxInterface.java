public class LibnyxInterface {
    static {
        System.loadLibrary("libnyx_middleman.so");
    }
    
    //Note each char is 16 bit = 64 bits total
    private char[] nyx_process = new char[4]; // 64 bit pointer stored in string, should only ever be accessed within c
    ...
    public void nyxNew() {
        this.nyxNew(this.sharedir, this.workdir, this.cpuID, this.inputBufferSize, this.inputBufferWriteProtection);
    }

    // Calls nyx_new to create the new nyx-vm instance
    // This launches a VM with your specified Snapshot starting point in the sharedir
    // sets the nyx_process to be used for all other functions
    private native void nyxNew(String sharedir, String workdir, int cpu_id, int input_buffer_size, boolean input_buffer_write_protection);
    ...
    // Lets the nyx-vm start the fuzzing loop, doesnt return until after checkpoint is reverted.
